...
test("server", (t) => {
  let server, client, sessionid, kickclient;
  const events = new Events();
  t.test("init", async (t) => {
    server = await Server(
      { port, host, channels: ["public", "private"] },
      { actions },
  ...
  t.test("close session", async (t) => {
    const sessions = await server.listSessions();
    await server.closeSession(sessions[0]);
